...
import org.fog.vmmigration.DecisionMigration_OLD;
...
public class AppExemplo2 {
	...
	public static void main(String[] args) throws Exception {
		...
		// Constar na dissertacao a politica de migracao deve estar alinhada com a politica de handoff. http://sbrc2016.ufba.br/downloads/WoCCES/155119.pdf
		// https://arxiv.org/pdf/1611.05539.pdf -> para referenciar muitos aspectos de fogComputing 
		//		Proximos steps:
		//			- verificar e talvez configurar a rede entre os ServerCloudlets
		//			- Buscar os valores reais de acordo com a literatura e colocar o link dos artigos como comentario na atribuição destes valores
		//			- pensar nos pontos de mediçao

		/* Alguns artigos sobre o projeto
		 * https://arxiv.org/pdf/1611.05539.pdf (Fog Computing: A Taxonomy, Survey and Future Directions)
		 * http://www.cs.wm.edu/~liqun/paper/hotweb15.pdf (Fog Computing: Platform and Applications)
		 * http://sbrc2016.ufba.br/downloads/WoCCES/155119.pdf (Avaliacao de desempenho de procedimentos de handoff em redes IPv6 e uma discussao sobre a viabilidade de aplicacao em sistemas criticos)
		 * http://ieeexplore.ieee.org/stamp/stamp.jsp?arnumber=7098039 (Fog Computing Micro Datacenter Based Dynamic Resource Estimation and Pricing Model for IoT)
		 * */
		//		verificar se eh viavel colocar a criacao dos appmodules na conexao com o ServerCloudlet.
...
//						setPositionScPolicy(Policies.RANDOM_SC_LOCATION);
		...
		for(FogDevice sc: getServerCloudlets()){
			for(FogDevice sc1 : getServerCloudlets()){
				...
				System.out.println("Delay between "+sc.getName()+" and "+sc1.getName()+": "+NetworkTopology.getDelay(sc.getId(), sc1.getId()));
...
//		Random rand = new Random(getSeed());
		for(MobileDevice st: getSmartThings()){// It only creates the virtual machine for each smartThing
			if(st.getSourceAp()!=null){
				CloudletScheduler cloudletScheduler = new TupleScheduler(500,1);      // CloudletSchedulerDynamicWorkload(500,1);
				...
				//						, getBrokerList().get(st.getMyId()).getId()//userId -> It should be the brokerId
				//						, 500//mips
			...
			for(MobileDevice st: getSmartThings()){
				if(st.getApDevices()!=null){
					if(sc.equals(st.getSourceServerCloudlet())){
						//						moduleMapping.addModuleToDevice("client"+i, sc.getName(), 1);//MaxAndMin.MAX_SMART_THING);// numOfDepts*numOfMobilesPerDept);
						//						moduleMapping.addModuleToDevice("connector"+i, sc.getName() ,1);// MaxAndMin.MAX_SMART_THING); // fixing all instances of the Connector module to cloudlets
						//						moduleMapping.addModuleToDevice("concentration_calculator"+i, sc.getName(), 1);//MaxAndMin.MAX_SMART_THING);
						...
						//	moduleMapping.addModuleToDevice("connector"+st.getMyId(), st.getName(), 1);
					}
				}
				i++;
			}
...
//			}
//			else {
//				LogMobile.debug("AppExemplo2.java", "POSITION ISN'T AVAILABLE... (AP) X ="+ coordX+ " Y = " +coordY+" Reallocating..." );		
//			}
//		}
	}
...
//		Random rand = new Random((Integer.MAX_VALUE*getSeed())/(i+1));
		...
		DeterministicDistribution distribution0 = new DeterministicDistribution(EEG_TRANSMISSION_TIME);//+(i*getRand().nextDouble()));

		Set<MobileSensor> sensors = new HashSet<>();

		MobileSensor sensor = new MobileSensor("Sensor"+i //Tuple's name -> ACHO QUE DÁ PARA USAR ESTE CONSTRUTOR 
				,"EEG"+i //Tuple's type
				,i //User Id
		...
		MobileActuator actuator0 = new MobileActuator("Actuator"+i, i, "MyApp_vr_game"+i, "DISPLAY"+i);
		...
		//				, null //geoLocation
		//				, "DISPLAY"+i
		//				, "DISPLAY"+i);  // ../..PAREI AQUI: POR QUE O ATUADOR ESTA RECEBENDO EM UM DISPLAY DIFERENTE
...
//				if(coordDevices.getPositions(coordX, coordY)==-1){//verify if it is empty
					...
					st = new MobileDevice("SmartThing"+ Integer.toString(i)
							, characteristics
							, vmAllocationPolicy// - seria a maquina que executa dentro do fogDevice?
							, storageList
...
//				}
//				else{
//					LogMobile.debug("AppExemplo2.java","POSITION ISN'T AVAILABLE... (ST)X ="+ coordX+ " Y = " +coordY+" Reallocating..."  );
//				}
			} 
		} catch (Exception e) {
...
//				}
//				else{
//					LogMobile.debug("AppExemplo2.java","POSITION ISN'T AVAILABLE... (SC)X ="+ coordX+ " Y = " +coordY+" Reallocating..."  );
//
//				}
//			}
...
//		Random rand = new Random(100*getSeed());
		for(FogDevice sc : serverCloudlets){//It makes a full graph 
			for(FogDevice sc1 : serverCloudlets){
				...
				if(sc.getUplinkBandwidth() < sc1.getDownlinkBandwidth()){
					...
					NetworkTopology.addLink(sc.getId(), sc1.getId(), sc.getUplinkBandwidth(),getRand().nextDouble());
				}
				else{
					net.put(sc1,sc1.getDownlinkBandwidth());
					NetworkTopology.addLink(sc.getId(), sc1.getId(), sc1.getDownlinkBandwidth(),getRand().nextDouble());
				}	
			}
	...
	private static Application createApplication(String appId, int userId, int myId, AppModule userVm){
		...
		//		application.addAppEdge(source, destination, tupleCpuLength, tupleNwLength, tupleType, direction, edgeType)
		...
		//		application.addAppEdge(source, destination, periodicity, tupleCpuLength, tupleNwLength, tupleType, direction, edgeType)?
	...
	private static Application createApplication(String appId, int userId, int myId){
		...
		application.addAppModule("client"+myId, 10); // adding module Client to the application model
		application.addAppModule("concentration_calculator"+myId, 10); // adding module Concentration Calculator to the application model
		application.addAppModule("connector"+myId, 10); // adding module Connector to the application model
		...
		//		application.addAppEdge(source, destination, tupleCpuLength, tupleNwLength, tupleType, direction, edgeType)
		application.addAppEdge("client"+myId, "concentration_calculator"+myId, 3500, 500, "_SENSOR", Tuple.UP, AppEdge.MODULE); // adding edge from Client to Concentration Calculator module carrying tuples of type _SENSOR
		//		application.addAppEdge(source, destination, periodicity, tupleCpuLength, tupleNwLength, tupleType, direction, edgeType)?
		application.addAppEdge("concentration_calculator"+myId, "connector"+myId, 1000, 1000, 1000, "PLAYER_GAME_STATE", Tuple.UP, AppEdge.MODULE); // adding periodic edge (period=1000ms) from Concentration Calculator to Connector module carrying tuples of type PLAYER_GAME_STATE

		application.addAppEdge("concentration_calculator"+myId, "client"+myId, 14, 500, "CONCENTRATION", Tuple.DOWN, AppEdge.MODULE);  // adding edge from Concentration Calculator to Client module carrying tuples of type CONCENTRATION
		application.addAppEdge("connector"+myId, "client"+myId, 1000, 28, 1000, "GLOBAL_GAME_STATE", Tuple.DOWN, AppEdge.MODULE); // adding periodic edge (period=1000ms) from Connector to Client module carrying tuples of type GLOBAL_GAME_STATE
		application.addAppEdge("client"+myId, "DISPLAY"+myId, 1000, 500, "SELF_STATE_UPDATE", Tuple.DOWN, AppEdge.ACTUATOR);  // adding edge from Client module to Display (actuator) carrying tuples of type SELF_STATE_UPDATE
		application.addAppEdge("client"+myId, "DISPLAY"+myId, 1000, 500, "GLOBAL_STATE_UPDATE", Tuple.DOWN, AppEdge.ACTUATOR);  // adding edge from Client module to Display (actuator) carrying tuples of type GLOBAL_STATE_UPDATE

		/*
		 * Defining the input-output relationships (represented by selectivity) of the application modules. 
		 */
		application.addTupleMapping("client"+myId, "EEG"+myId, "_SENSOR", new FractionalSelectivity(0.9)); // 0.9 tuples of type _SENSOR are emitted by Client module per incoming tuple of type EEG
		application.addTupleMapping("client"+myId, "CONCENTRATION", "SELF_STATE_UPDATE", new FractionalSelectivity(1.0)); // 1.0 tuples of type SELF_STATE_UPDATE are emitted by Client module per incoming tuple of type CONCENTRATION 
		application.addTupleMapping("concentration_calculator"+myId, "_SENSOR", "CONCENTRATION", new FractionalSelectivity(1.0)); // 1.0 tuples of type CONCENTRATION are emitted by Concentration Calculator module per incoming tuple of type _SENSOR 
		application.addTupleMapping("client"+myId, "GLOBAL_GAME_STATE", "GLOBAL_STATE_UPDATE", new FractionalSelectivity(1.0)); // 1.0 tuples of type GLOBAL_STATE_UPDATE are emitted by Client module per incoming tuple of type GLOBAL_GAME_STATE 
